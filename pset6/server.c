...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            int pos1 = firstSpace - line;
            
            secondSpace = strchr(firstSpace + 1,' ');
            int pos2 = secondSpace - line;
            
            int i;
            for(i=0;i<firstSpace - line;i++){
                method[i] = line[i];
            }
            method[i] = '\0';
            
            char rTarget[pos2 - pos1];
            ...
            char query[10000];
            int lenPath = strlen(rTarget);
            char aPath[lenPath + 1];
            strcpy(query,"");
            char* qmark;
            qmark = strchr(rTarget,'?');
            
            if(qmark != NULL){
                int qpos = qmark - rTarget;
                lenPath = qpos;
                if(rTarget[qpos+1] != '\0'){
                    int len = strlen(rTarget) - qpos;
                    for(i = 0,j = qpos + 1;i < len;i++,j++){
                        query[i] = rTarget[j];
                    }
                    query[i] = '\0';
                }
            ...
            for(i = 0,j = 0 ; i < lenPath ; i++,j++){
                aPath[i] = rTarget[j];
            }
            aPath[i] = '\0';
            
            
            if(strstr(method," ") != NULL || strstr(rTarget," ") != NULL || strstr(hType," ") != NULL ){
                error(400);
                continue;
            }
            ...
            if(strcmp(hType,"HTTP/1.1") != 0 ){
                error(505);
                continue;
            }
            
            if(strchr(aPath,'.') == NULL ){
                error(501);
                continue;
            }
            ...
            char path[strlen(root) + strlen(aPath) + 1];
            strcpy(path,root);
            strcat(path,aPath);
            ...
            char* dot;
            dot = strrchr(aPath,'.');
            int posDot = dot - aPath;
            
            for(i = 0,j = posDot + 1 ; aPath[j]!='\0';i++,j++){
                extension[i] = tolower(aPath[j]);
            }
            extension[i] = '\0';
