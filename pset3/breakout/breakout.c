...
int main(void)
{
    ...
    while (lives > 0 && bricks > 0)
    {
        ...
        if(getX(ball) + getWidth(ball) >= getWidth(window)){
            ...
            xvelocity = -xvelocity;
        }else if(getY(ball)<=0 || object == paddle){
            yvelocity= -yvelocity;
...
GOval initBall(GWindow window)
{
    GOval oval = newGOval((WIDTH - (RADIUS * 2)) / 2,(HEIGHT - (RADIUS * 2)) / 2,RADIUS * 2,RADIUS * 2);
    setFilled(oval,true);
    setColor(oval,"BLACK");
    add(window,oval);
    return oval;
...
GRect initPaddle(GWindow window)
{
    GRect rect = newGRect( ( WIDTH - PWIDTH ) / 2, YPADDLE, PWIDTH, PHEIGHT);
    setFilled(rect,true);
    setColor(rect,"BLACK");
    add(window,rect);
    return rect;
